...
export default function Dashboard() {
  ...
  if (role === null) return <div style={{ padding: 40 }}>Loading...</div>;
  ...
  return (
    <div style={{ display: 'flex', fontFamily: 'Segoe UI, Roboto, sans-serif' }}>
      ...
      <div style={{
        ...
        zIndex: 1201, display: 'flex', alignItems: 'center', justifyContent: 'space-between',
        padding: '0 24px', boxShadow: '0 4px 20px rgba(102, 126, 234, 0.3)'
      }}>
      ...
      {/* Sidebar */}
      <div style={{
        width: drawerWidth, position: 'fixed', left: 0, top: '64px', bottom: 0,
        ...
        <div style={{ padding: '20px 0' }}>
          {menuItems.map((item, index) => (
            <div
              ...
              style={{
                display: 'flex', alignItems: 'center', padding: '12px 20px', margin: '4px 12px',
                borderRadius: '12px', cursor: 'pointer', transition: 'all 0.3s ease',
                background: selectedMenu === index ? 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' : 'transparent',
      ...
      {/* Main Content */}
      <div style={{
        marginLeft: drawerWidth, paddingTop: '64px', minHeight: '100vh',
        background: 'linear-gradient(135deg, #f8fafc 0%, #e2e8f0 100%)', flexGrow: 1
      }}>
        <div style={{ padding: '24px' }}>
          {menuItems[selectedMenu]?.component || <div>Pilih menu di sidebar.</div>}
        </div>
      </div>
    </div>
  );
